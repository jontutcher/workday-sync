...
def _write_absence_xlsx(path: Path, user_name: str, rows: list[tuple]) -> None:
...
_TEST_ROWS = [
    ...
    # Full day, with a comment (no AM/PM keyword)
...
def sample_xlsx(tmp_path_factory: pytest.TempPathFactory) -> Path:
    ...
    _write_absence_xlsx(out, _TEST_USER, _TEST_ROWS)
    return out
